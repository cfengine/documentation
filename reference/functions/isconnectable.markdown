...
This function checks whether a `hostname`:`port` is connectable within `timeout` in ms, before it times out. If `timeout` is 0, the connection blocks forever. `timeout` defaults to 100 ms.
