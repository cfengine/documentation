...
1. We recommend selecting dedicated interface used for PostgreSQL replication and optionally one for heartbeat.
2. We recommend having one shared IP address assigned for interface where MP is accessible (optionally) and one where PostgreSQL replication is configured (mandatory).
3. Both active and passive hub machines must be configured so that host names are different.
4. Both hubs must be placed in */etc/hosts* so that each will be accessible using its host-names.

### Example configuration used in this tutorial ###

In this tutorial we are using following configuration:
...
```
4. Run corosyn and pacemaker to check if both cluster nodes are seen each other:
```bash
crm_mon -Afr1
```
