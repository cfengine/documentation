...
in [`body hub control`][component-cf-hub#control-promises].
...
`cf-hub` can skip hosts for a few reasons, for example if they are in [`exclude_hosts`][component-cf-hub#exclude_hosts], or if it has reached the license count.
