...
bundle agent create_aout
{
  ...
  vars:
    ...
    "rmaout"
      string => execresult(
        "$(global_vars.rmexec) $(global_vars.aoutexec)", "noshell"
      );

    doesfileacexist::
      "compilestr"
        string => "$(global_vars.gccexec) $(global_vars.workdir)/a.c -o $(global_vars.aoutexec)";
...
{
  files:
    "$(global_vars.file2)" copy_from => local_cp("$(global_vars.file1)");
...
bundle agent stat
{
  ...
  vars:
    doesfile1exist::
      ...
      "file1_stat"
        string => execresult("/usr/bin/stat -c \"%y\" $(file1)", "noshell");
      ...
      "file2_stat"
        string => execresult("/usr/bin/stat -c \"%y\" $(file2)", "noshell");
  ...
  classes:
    "ok"
      expression => isgreaterthan(
        filestat("$(global_vars.file2)", "mtime"),
        filestat("$(global_vars.file1)", "mtime")
      );

    "newer"
      expression => isnewerthan("$(global_vars.file2)", "$(global_vars.file1)");
