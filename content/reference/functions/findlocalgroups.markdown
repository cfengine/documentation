...
The `filter` argument can be used to look up groups with specific attributes that match values. The filter is a `"data"` container or `"slist"` comprised of pairs of attribute and value/regex pattern `{ "attribute1=value1", "attribute2=value2", ... }`.
...
bundle agent main
{
  vars:
    ...
    "indices" slist => getindices("@(groups)");
...
where the format of the output looks like that
